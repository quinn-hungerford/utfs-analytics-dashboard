...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      tabsetPanel(id = "tabs",
                  tabPanel("Table View", uiOutput("tableHeader"), dataTableOutput("dataTable")),
...
server <- function(input, output, session) {
  ...
  # Route the data
  dataset <- reactive({
    if (input$dataset == "Sales Performance" && !is.null(input$salesDate)) {
      if (input$salesDate == "Sales_Summary_2425") {
        load_data("Sales_Performance_Past.xlsx", sheet = "Sales_Summary_2425")
      } else {
        load_data("Sales_Performance_Past.xlsx", sheet = input$salesDate)
      }
    } else if (input$dataset == "Volunteer Engagement") {
      read_all_sheets("Volunteer_Engagement_Past.xlsx")
    } else {
      load_data("Market_Attendance_Past.xlsx")
    }
  })
  ...
  output$veDateSelector <- renderUI({
    if (input$dataset == "Volunteer Engagement") {
      sheets <- tryCatch(readxl::excel_sheets("Volunteer_Engagement_Past.xlsx"), 
                         error = function(e) character(0))
      raw_labels <- sub(".*?(\\d{4})[-_](\\d{2})[-_](\\d{2}).*$", "\\1-\\2-\\3", sheets)
      df_choices <- data.frame(sheet = sheets, 
                               label = gsub("_","-", raw_labels), 
      ...
      selectInput(
        "veSheet", 
        "Select a Market Date (Table View Only):", 
        choices = choice_vec, 
  ...
  output$salesDateSelector <- renderUI({
    if (input$dataset == "Sales Performance") {
      ...
      choice_vec <- c("24–25 Sales Summary" = "Sales_Summary_2425", date_choice_vec)
      selectInput(
        "salesDate",
        "Select a Market Date or View 24–25 Sales Summary:",
        choices = choice_vec,
        selected = "Sales_Summary_2425"
      )
    }
  })
  ...
  output$dataTable <- renderDataTable({
    ...
    if (input$dataset == "Volunteer Engagement") {
      req(input$veSheet)
      df <- load_data("Volunteer_Engagement_Past.xlsx", sheet = input$veSheet)
    ...
    # Market Attendance table view
      ...
      attn_all <- load_data("Market_Attendance_Past.xlsx") %>%
      ...
      wanted <- c("Market Date",
                  "Total Customer Attendance",
                  "Average Temperature",
                  "Weather Condition",
                  "Number of Tabling Organizations",
                  "Number of Vendors",
                  "Notes")
    ...
    } else if (input$dataset == "Sales Performance" && input$salesDate == "Sales_Summary_2425") {
      df <- dataset() %>%
        mutate(`Market Date` = as.Date(`Market Date`))
  ...
  # Creating descriptive headers for each of the summary statistics sections
  ...
  output$forecastTable <- DT::renderDataTable({
    ...
    # Forecasting sales performance for Fall 2025 (summary page, general forecast)
      ...
      if (is_summary) {
        sales_summary <- load_data("Sales_Performance_Past.xlsx", sheet = "Sales_Summary_2425") %>%
          mutate(`Market Date` = as.Date(`Market Date`))
      ...
      # Forecasting sales performance for Fall 2025 (market page, category forecast)
        ...
        rows <- lapply(UPCOMING_MARKETS_2526, function(target_date) {
          ...
          per_market <- tryCatch(load_data("Sales_Performance_Past.xlsx", sheet = nearest_sheet), error = function(e) NULL)
          if (is.null(per_market)) {
            return(tibble(`Future Market` = target_date,
          ...
          per_market %>%
            group_by(Category) %>%
            summarise(`Expected Total Items Sold` = sum(`Number Sold`, na.rm = TRUE),
        ...
        df$Category <- factor(df$Category, levels = unique(c(requested_order, setdiff(df$Category, requested_order))))

        df <- df %>% 
          arrange(`Future Market`, Category)
        ...
        DT::datatable(
          ...
          DT::formatCurrency("Expected Gross Sales", currency = "$", interval = 3, mark = ",", digits = 2) |>
          DT::formatRound("Expected Total Items Sold", digits = 0, interval = 3, mark = ",")
      }
    }
  })
  ...
  output$summaryStatsTable <- DT::renderDataTable({
    ...
    # Summary statistics on market attendance (24-25) 
    } else if (input$dataset == "Market Attendance") {
      validate(
    ...
    # Summary statistics on sales performance (24-25) for each market date
               ...
               (input$salesDate %in% c("Sales_2024-09-11","Sales_2024-10-02","Sales_2024-10-16",
                                       "Sales_2024-10-30","Sales_2024-11-13",
                                       "Sales_2025-01-29","Sales_2025-02-12","Sales_2025-02-26",
                                       "Sales_2025-03-12","Sales_2025-04-02","Sales_2025-04-16"))) {
